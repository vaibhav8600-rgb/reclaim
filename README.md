...
- **Understanding and patterns:** each injury page explains the condition in plain words with sources (disc bulge, tendon, ligament sprain or tear, back or neck pain), and shows patterns from your own days — e.g. pain the day after fewer steps, a short night or a rehab session.
- **Records, clearer:** photos of a multi-page report become one record, read together, with each finding's page; a record shows what changed since the previous one of the same kind; medicines are listed once each with their latest dose. **Settings → AI → Show What's Sent First** shows exactly what each AI request carries, and what it never does, before anything leaves the phone.
- **Fitness:** strength, cardio and mobility beside rehab — workouts with a rest timer, effort and personal bests. Every exercise says how it sits with your injuries: a tip to adjust it, or "ask your physio first" for heavy loads on a disc, neck or knee problem.
